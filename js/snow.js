    ...
    if(ns6up){
        ...
        doc_width=iecompattest().clientWidth;doc_height=iecompattest().clientHeight
    }
    dx=new Array();
    xp=new Array();
    yp=new Array();
    ...
    for(i=0;i<no;++i){
        ...
        if(ie4up||ns6up){
            document.write(
                '<div id="dot'+i+'" style="POSITION:fixed;Z-INDEX:-1;VISIBILITY:visible;TOP:15px;LEFT:15px;"><span style="font-size:15px;color:yellow">🎁</span></div>'
                )
            }
        }
    function snowIE_NS6(){
        ...
        snowtimer=setTimeout('snowIE_NS6()',1)
    }
    function hidesnow(){
        if(window.snowtimer){
